...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      images: [{ url: tour.image || "/og-default.jpg", width: 1200, height: 630, alt: tour.title }],
    },
    twitter: {
      card: "summary_large_image",
      title,
      description,
      images: [tour.image || "/og-default.jpg"],
    },
  };
}
...
export default async function TourDetailsPage({ params }) {
  ...
  const tourSchema = {
    ...
    url: `https://elkhazany-tour.vercel.app/tours/${tour.slug}`,
    provider: {
      "@type": "TravelAgency",
                  ...
                  {tour.packages?.map((pkg) => (
                    ...
                    >
                      ...
                      <div className="mt-6 flex gap-3">
                        <Link
                          href={`/booking?tour=${encodeURIComponent(tour.title)}&package=${encodeURIComponent(pkg.name)}`}
