...
function detailsBlock({ data }) {
  ...
  const rows = [
    ...
    ["Guests", data?.people || data?.guests || ""],
  ...
  const tableRows = rows
    .map(
      ([k, v]) => `
      <tr>
        <td style="padding:10px 12px;border-bottom:1px solid rgba(255,255,255,.06);color:rgba(255,255,255,.65);width:170px">${esc(
          k
        )}</td>
        <td style="padding:10px 12px;border-bottom:1px solid rgba(255,255,255,.06);color:#fff;font-weight:600">${esc(
          v
        )}</td>
      </tr>`
    )
...
// ✅ Special template: Confirmed email (used by bookings/status route)
